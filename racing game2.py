...
# --- STATS ---
CHASSIS_STATS = {
    "F1":       {"base_spd": 19.5, "accel": 0.6, "turn": 2.4, "base_grip": 0.99, "mass": 800},
    "SUPER":    {"base_spd": 20.0, "accel": 0.55, "turn": 2.0, "base_grip": 0.96, "mass": 1400},
    "NASCAR":   {"base_spd": 18.5, "accel": 0.50, "turn": 1.8, "base_grip": 0.97, "mass": 1800},
    "LE_MANS":  {"base_spd": 16.0, "accel": 0.45, "turn": 1.9, "base_grip": 0.98, "mass": 1000},
    "DRIFT":    {"base_spd": 15.0, "accel": 0.7, "turn": 2.5, "base_grip": 0.90, "mass": 1200},
}
...
class AssetManager:
    def __init__(self, screen):
        ...
        self.car_sprites = {}
        self.car_previews = {}
        self.scenery_objects = [] # Stores tree positions
        
        # Loading Screen
        self.screen.fill(BLACK)
        txt = self.font_header.render("BUILDING TRACK...", True, NEON_ORANGE)
        ...
        # --- LOAD TREE IMAGE ---
        # Try loading tree.png or tree.jpg
        self.tree_img = None
    ...
    def aggressive_clean_image(self, image):
        ...
        for x in range(width):
            for y in range(height):
                ...
                # Treat very light colors as transparent background
    ...
    def catmull_rom_spline(self, points, steps=30):
        # Increased steps for smoother curves
    ...
    def paint_track(self, surface, color, points, width):
        # "Paint Brush" method: Draws circles at every point for 100% smooth edges
        radius = width // 2
        for p in points:
            pygame.draw.circle(surface, color, (int(p[0]), int(p[1])), radius)

    def generate_scenery(self, mask_surface, num_trees):
        # We look for BLACK pixels (Grass) to place trees
    ...
    def generate_procedural_track(self):
        ...
        # --- TRACK LAYOUT (Your custom points) ---
        points = [
            (4080, 2250), (4080, 3862), (4080, 4387), (3802, 4987),
            (3045, 5130), (2325, 4837), (2190, 4155), (1267, 3435),
            (862, 3277), (525, 2745), (547, 1965), (982, 1380),
            (1477, 1012), (2370, 675), (3180, 577), (3690, 652),
            (3885, 817), (3990, 945), (4080, 2250),
        ]
        ...
        self.paint_track(mask_surf, WHITE, smooth_points, TRACK_WIDTH)

        self.paint_track(mask_surf, WHITE, smooth_points, 550)
        
        # --- DISTANCE-BASED DASHED LINES ---
        dash_length = 80   
        gap_length = 80    
        ...
        cropped_vis = vis.subsurface(crop_rect)
        minimap = pygame.transform.smoothscale(cropped_vis, (250, 250))
...
class SoundManager:
    def __init__(self):
        pygame.mixer.init()
        self.sounds = {}
        files = {"engine": "engine_f1.wav", "drift": "skid.wav", "crash": "crash.wav"}
...
class Car:
    ...
    def update(self, mask_surface, meta_data):
        ...
        if throttle > 0:
            ...
            if random.randint(0, 100) < 5: self.audio.play("engine")
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Speed Show - Smooth Edition")
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and self.state == "RACE" and event.key == pygame.K_ESCAPE: self.state = "MENU"
    ...
    def draw_menu(self, mx, my):
        ...
        draw_text(self.screen, "SMOOTH EDITION", self.assets.font_ui, WHITE, SCREEN_WIDTH//2, 220, True)
    ...
    def draw_race(self):
        ...
        if self.assets.tree_img:
            for tree in self.assets.scenery_objects:
                tx, ty = tree['pos']
                # Optimization: Only draw if close to camera
    ...
    def draw_minimap(self):
        ...
        draw_glass_panel(self.screen, map_x-5, map_y-5, map_w+10, map_h+10, BLACK)
        self.screen.blit(track_mini, (map_x, map_y))
        
        scale_x = map_w / real_w 
        scale_y = map_h / real_h
        
        # Offset the dots to match the cropped map
